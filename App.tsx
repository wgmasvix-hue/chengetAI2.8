...
const FullLogo = ({ className = "" }: { className?: string }) => (
  <div className={`flex flex-col items-center gap-4 ${className}`}>
    ...
    <h2 className="text-3xl font-black tracking-tighter text-[#136f6f] google-sans uppercase">Bulawayo Polytechnic</h2>
    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest -mt-2">Institutional Repository</p>
  </div>
);
...
const Overview: React.FC<{ setView: (v: AppView) => void, userRole?: string }> = ({ setView, userRole }) => {
  const [news, setNews] = useState<string>('Bulawayo Polytechnic Institutional Repository — preserving and sharing Zimbabwe\'s knowledge...');
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const res = await generateSearchGroundedResponse("Summarize why open-access institutional repositories matter for polytechnic colleges in Zimbabwe in one sentence.");
        setNews(res.text || "Empowering Bulawayo Polytechnic students and staff with open access to academic knowledge.");
      } catch (e) {
        setNews("Bulawayo Polytechnic IR — Open access to research, theses, and institutional knowledge.");
      }
    };
    fetchNews();
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-center gap-6 mb-16 animate-in fade-in slide-in-from-top-10 duration-1000">
            <div className="flex flex-wrap items-center gap-3">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-zinc-900 text-white text-[10px] font-black rounded-full tracking-[0.2em] uppercase shadow-xl">IR Online</div>
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-teal-50 border border-teal-100 text-[#136f6f] text-[10px] font-black rounded-full tracking-[0.2em] uppercase">DSpace 7.6 · bpoly.ac.zw</div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-8 animate-in fade-in slide-in-from-left-10 duration-1000 delay-200">
              <h1 className="text-4xl md:text-[5.5rem] font-black google-sans mb-4 leading-[0.9] text-zinc-900 tracking-tighter uppercase">
                BULAWAYO<br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#136f6f] to-teal-400">POLYTECHNIC</span><br />
                LIBRARY.
              </h1>
              <p className="text-2xl text-zinc-500 max-w-2xl mb-16 leading-relaxed font-medium">Bulawayo Polytechnic's institutional repository — open access to theses, research, course materials, and the college's academic heritage.</p>
              ...
              <div className="flex flex-wrap gap-6">
                <button onClick={() => setView(userRole === 'student' ? AppView.STUDENT_PORTAL : AppView.ASSISTANT)} className="group px-12 py-7 bg-[#136f6f] hover:bg-[#1a8b8b] text-white rounded-[2rem] font-black transition-all shadow-2xl shadow-teal-900/30 flex items-center gap-6 text-sm uppercase tracking-widest active:scale-95">
                  <span className="group-hover:rotate-12 transition-transform text-2xl">📚</span> {userRole === 'student' ? 'My Learning Space' : 'Browse Repository'}
                </button>
                <button onClick={() => setView(AppView.DIGITAL_RESOURCES)} className="px-12 py-7 bg-white hover:bg-zinc-50 text-zinc-900 border border-zinc-200 rounded-[2rem] font-black transition-all flex items-center gap-6 shadow-xl text-sm uppercase tracking-widest active:scale-95">
                  <span className="opacity-50 text-2xl">🗄️</span> Digital Archive
                </button>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 hidden lg:flex flex-col gap-6 animate-in fade-in slide-in-from-right-10 duration-1000 delay-500">
               <div className="bg-zinc-900 p-8 rounded-[3rem] shadow-2xl text-white border border-white/5 relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-6">Recently Added to Repository</p>
      ...
      <section className="px-6 py-32 md:px-16 bg-white relative">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24">
            <h2 className="text-xs font-black text-[#136f6f] uppercase tracking-[0.5em] mb-4">Repository Services</h2>
            <h3 className="text-5xl font-black google-sans text-zinc-900 tracking-tighter uppercase">What the BPoly IR Offers</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard
              ...
              icon="🗄️"
              desc="Preserve and discover Bulawayo Polytechnic theses, research papers, reports, and course materials in a permanent open-access repository."
              specs={['Student Theses', 'Staff Research', 'Course Materials']}
            />
            <FeatureCard
              title="AI Research Assistant"
              icon="🤖"
              desc="TAURAI — an AI librarian grounded in BPoly's own collection — answers research questions, suggests sources, and helps cite materials correctly."
              specs={['Instant Q&A', 'Citation Help', 'Collection Search']}
            />
            <FeatureCard
              title="Library Analytics"
              icon="📊"
              desc="Real-time usage statistics, collection health reports, and borrowing trends to help the BPoly library team serve students better."
              specs={['Usage Dashboards', 'Collection Gaps', 'Koha Integration']}
            />
          </div>
        </div>
      </section>
